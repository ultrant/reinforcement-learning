...
class DeepQNetworkP2:
    ...
    def graph_pre(self):
        ...
        checkpoint = tf.train.latest_checkpoint('./checkpoints_p2')
        if checkpoint != None and self.mode == 'use mode':
            print('P2 Restore Checkpoint %s' % (checkpoint))
          ...
          #  chkp.print_tensors_in_checkpoint_file(checkpoint, tensor_name='Variable_9', all_tensors=False)
           ...
           # print(self.sess.run(self.out))
            print("P2 Initialized new Graph")
    ...
    def learn(self,step):
       ...
       # out_batch = out.eval(feed_dict={inp: inp_t1_batch})
        ...
        if step % self.SAVE_STEP == 0:
            self.saver.save(self.sess, './checkpoints_p2/model.ckpt', global_step=step)
        #if step % 100 == 0:
