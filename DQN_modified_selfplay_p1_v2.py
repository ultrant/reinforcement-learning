...
class DeepQNetworkP1:
    ...
    def graph_pre(self):
        ...
        if checkpoint != None and self.mode == 'use mode':
            print('P1 Restore Checkpoint  %s' % (checkpoint))
          ...
          #  chkp.print_tensors_in_checkpoint_file(checkpoint, tensor_name='Variable_9', all_tensors=False)
           ...
           # print(self.sess.run(self.out))
            print("P1 Initialized new Graph")
    ...
    def learn(self,step):
       ...
       # out_batch = out.eval(feed_dict={inp: inp_t1_batch})
        ...
        if step % self.SAVE_STEP == 0:
            self.saver.save(self.sess, './checkpoints_p1/model.ckpt', global_step=step)
        #if step % 100 == 0:
    ...
    def plt_data(self,image_path, win_rate_list_p1, win_rate_list_p2, avg_step_p1_list, avg_step_p2_list, reward_p1_list, reward_p2_list, loss_p1_list, loss_p2_list):
        plt.figure(1)
        plt.plot(np.arange(len(win_rate_list_p1)), win_rate_list_p1, label='player1_win_rate(AI)')
       ...
       # figure;
        plt.figure(2)
        plt.plot(np.arange(len(avg_step_p1_list)), avg_step_p1_list, label='player1_aver_step(AI)')
        ...
        plt.show()
        plt.figure(3)
        plt.plot(np.arange(len(reward_p1_list)), reward_p1_list, label='player1_reward_total(AI)')
        ...
        plt.show()
        plt.figure(4)
        plt.plot(np.arange(len(loss_p1_list)), loss_p1_list, label='player1_loss(AI)')
